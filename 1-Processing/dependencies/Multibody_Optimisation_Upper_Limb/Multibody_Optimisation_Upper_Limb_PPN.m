...
% 'P': sphere-on-sphere (5 degrees of freedom) at glenohumeral
% 'P': constant clavicle
% 'N': no constraint (6 degrees of freedom) at scapulothoracic
%
% REFERENCES
% A. El Habachi, S. Duprey, L. Cheze, R. Dumas. A parallel mechanism of the  
% shoulder—application to multi-body optimisation. Multibody System  
% Dynamics 2015, 33(4): 439–451.
%__________________________________________________________________________
%
% CALLED FUNCTIONS
...
lambdakG = zeros(1,1,n); % Parallel constraint
...
% -------------------------------------------------------------------------
% Newton-Raphson
while max(permute(sqrt(sum(F.^2)),[3,2,1])) > 10e-12 && step < 150
    ...
    % (rD2 - rP1)^2 - (d1)^2 = 0
    phikG = dot(Segment(2).Q(7:9,1,:) - Segment(1).Q(4:6,1,:),Segment(2).Q(7:9,1,:) - Segment(1).Q(4:6,1,:)) - repmat((Joint(1).d(1,1))^2,[1,1,n]);
    % Jacobian of kinematic constraints
    KkG = zeros(1,3*12,n); % Initialisation
    KkG(1,4:6,:) = -2*permute(Segment(2).Q(7:9,1,:) - Segment(1).Q(4:6,1,:),[2,1,3]);
    KkG(1,19:21,:) = 2*permute(Segment(2).Q(7:9,1,:) - Segment(1).Q(4:6,1,:),[2,1,3]);
    ...
    dKlambdakGdQ(4:6,4:6,:) = Mprod_array3(lambdakG,repmat(2*eye(3),[1,1,n]));
    dKlambdakGdQ(4:6,19:21,:) = Mprod_array3(lambdakG,repmat(-2*eye(3),[1,1,n]));
    dKlambdakGdQ(19:21,4:6,:) = Mprod_array3(lambdakG,repmat(-2*eye(3),[1,1,n]));
    dKlambdakGdQ(19:21,19:21,:) = Mprod_array3(lambdakG,repmat(2*eye(3),[1,1,n]));
   ...
   % Assembly
    ...
    lambdakG = lambdakG + dX(37,1,:);
    lambdakC = lambdakC + dX(38,1,:);
    % lambdakT: no constraint
    lambdar = lambdar + dX(39:end,1,:);
