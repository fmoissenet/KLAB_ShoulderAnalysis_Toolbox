% FUNCTION
% Multibody_Optimisation_Upper_Limb_LmPN.m
...
% 'Lm': minimized "ligament" length variations at glenohumeral
% 'P': constant clavicle
...
function [Segment] = Multibody_Optimisation_Upper_Limb_LmPN(Segment,Joint)
...
%     Kk = [];  % Jacobian of kinematic constraints
   ...
   % Assembly
    phikCT = [phikC;phikT];
    KkCT = [KkC;KkT];
    lambdakCT = [lambdakC;lambdakT];
    dKlambdakCTdQ = dKlambdakCdQ + dKlambdakTdQ;
    ...
    % Assembly
    phimphikG = [phim;phikG];
    KmKkG = [Km;KkG];    

    % Display errors
    Mean_phikCT = mean(Mprod_array3(permute(phikCT,[2,1,3]),phikCT),3)
    Mean_phikG = mean(Mprod_array3(permute(phikG,[2,1,3]),phikG),3)
    Mean_phir = mean(Mprod_array3(permute(phir,[2,1,3]),phir),3)
    Mean_phim = mean(Mprod_array3(permute(phim,[2,1,3]),phim),3)
    
    % Weight factors on the glenohumeral kinematic constraints
    % Weight matrix
    wm = ones((size(Segment(1).rM,2) + ...
        size(Segment(2).rM,2) + ...
        size(Segment(3).rM,2))*3,1);
    wkG = [5;5;5]; % Weight factors on the glenohumeral constraints
    WmkG = repmat(diag([wm;wkG]),[1,1,n]);
    
    % The choice of weight factors can be modified    
    % wkG = [5;5;5]; for Work_v7
    % wkG = [2;2;2]; for Work_v8
    ...
    % Compute dX
    % dX = inv(-dF/dx)*F(X)
    % F(X) = [KmKkG'*W*phimphikG + [KkCT;Kr]'*[lambdakCT;lambdar];[phikCT;phir]]
    % X = [Q;[lambdakCT;lambdar]]
    F = [Mprod_array3(permute(KmKkG,[2,1,3]), Mprod_array3(WmkG,phimphikG)) + ...
        Mprod_array3(permute([KkCT;Kr],[2,1,3]), [lambdakCT;lambdar]); ...
        [phikCT;phir]]; % with transpose = permute( ,[2,1,3])
    dFdX = [Mprod_array3(permute(KmKkG,[2,1,3]),Mprod_array3(WmkG,KmKkG)) + ...
        dKlambdakCTdQ + dKlambdardQ, permute([KkCT;Kr],[2,1,3]); ...
        [KkCT;Kr],zeros(size([KkCT;Kr],1),size([KkCT;Kr],1),n)];
    ...
    % lambdakG: no constraint
    lambdakC = lambdakC + dX(37,1,:);
    % lambdakT: no constraint
    lambdar = lambdar + dX(38:end,1,:);
