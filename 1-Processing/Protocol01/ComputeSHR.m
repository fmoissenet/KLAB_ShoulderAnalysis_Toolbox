...
if contains(c3dFiles.name,'ANALYTIC2') || contains(c3dFiles.name,'ANALYTIC1') % Only applied on elevation tasks
    ...
    for icycle = 1:size(Trial.Joint(1).Euler.cycle,4)
        ...
        if contains(c3dFiles.name,'ANALYTIC2')
            imax = find(abs(Trial.Joint(1).Euler.cycle(:,1,:,icycle))==max(abs(Trial.Joint(1).Euler.cycle(:,1,:,icycle))));
            imin = find(abs(Trial.Joint(1).Euler.full(:,1,:))==min(abs(Trial.Joint(1).Euler.full(:,1,:))));
        elseif contains(c3dFiles.name,'ANALYTIC1')
            imax = find(abs(Trial.Joint(1).Euler.cycle(:,3,:,icycle))==max(abs(Trial.Joint(1).Euler.cycle(:,3,:,icycle))));
        ...
        Trial.SHR(1).theta_HT(1).cycle(1,1,:,icycle)  = permute(theta_HT1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(1).theta_ST(1).cycle(1,1,:,icycle)  = permute(theta_ST1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(1).theta_GH(1).cycle(1,1,:,icycle)  = permute(theta_GH1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(1).SHR_curve(1).cycle(1,1,:,icycle) = permute(SHR1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(1).SHR_mean(1).cycle(1,1,:,icycle)  = mean(SHR1,'omitnan'); % scalar
        % RETURN
        clear range theta_HT1 theta_ST1 theta_GH1 SHR1;
        % Compute best-fit polynomial curves (3th order) between 30° and 90°
        angleMin              = 30;
        ...
        % Store results
        Trial.SHR(1).label                            = 'Right scapulo-humeral rhythm';
        Trial.SHR(1).theta_HT(2).cycle(1,1,:,icycle)  = permute(theta_HT1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(1).theta_ST(2).cycle(1,1,:,icycle)  = permute(theta_ST1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(1).theta_GH(2).cycle(1,1,:,icycle)  = permute(theta_GH1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(1).SHR_curve(2).cycle(1,1,:,icycle) = permute(SHR1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(1).SHR_mean(2).cycle(1,1,:,icycle)  = mean(SHR1,'omitnan'); % scalar
    end
    ...
    for icycle = 1:size(Trial.Joint(6).Euler.cycle,4)
        % Set frames of interest
        if contains(c3dFiles.name,'ANALYTIC2')
            imax = find(abs(Trial.Joint(6).Euler.cycle(:,1,:,icycle))==max(abs(Trial.Joint(6).Euler.cycle(:,1,:,icycle))));
            imin = find(abs(Trial.Joint(6).Euler.full(:,1,:))==min(abs(Trial.Joint(6).Euler.full(:,1,:))));
        elseif contains(c3dFiles.name,'ANALYTIC1')
            imax = find(abs(Trial.Joint(6).Euler.cycle(:,3,:,icycle))==max(abs(Trial.Joint(6).Euler.cycle(:,3,:,icycle))));
        ...
        Trial.SHR(2).theta_HT(1).cycle(1,1,:,icycle)  = permute(theta_HT1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(2).theta_ST(1).cycle(1,1,:,icycle)  = permute(theta_ST1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(2).theta_GH(1).cycle(1,1,:,icycle)  = permute(theta_GH1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(2).SHR_curve(1).cycle(1,1,:,icycle) = permute(SHR1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(2).SHR_mean(1).cycle(1,1,:,icycle)  = mean(SHR1,'omitnan'); % scalar
        % RETURN
        clear range theta_HT1 theta_ST1 theta_GH1 SHR1;
        % Compute best-fit polynomial curves (3th order) between 30° and 90°
        angleMin              = 30;
        ...
        % Store results
        Trial.SHR(2).label                            = 'Left scapulo-humeral rhythm';
        Trial.SHR(2).theta_HT(2).cycle(1,1,:,icycle)  = permute(theta_HT1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(2).theta_ST(2).cycle(1,1,:,icycle)  = permute(theta_ST1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(2).theta_GH(2).cycle(1,1,:,icycle)  = permute(theta_GH1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(2).SHR_curve(2).cycle(1,1,:,icycle) = permute(SHR1,[2,3,1]); % 61 point vector (30° to 90°)
        Trial.SHR(2).SHR_mean(2).cycle(1,1,:,icycle)  = mean(SHR1,'omitnan'); % scalar
    end
end