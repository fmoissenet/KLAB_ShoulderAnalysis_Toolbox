...
% -------------------------------------------------------------------------
% Dependencies : - 3D Kinematics and Inverse Dynamics toolbox by Raphaël Dumas: 
...
% the first 50 frames of each trial.

% Euler sequence YXZ (Wu et al. 2005) :
%   DOF1 Y : axial rotation    (+= rotation toward right)
%   DOF2 X : lateral tilt      (+= tilt toward right)
%   DOF3 Z : kyphosis/flexion  (+= anterior flexion)
...
if contains(c3dFiles.name,'ANALYTIC') || contains(c3dFiles.name,'FUNCTIONAL') || contains(c3dFiles.name,'CALIBRATION3') % Add for unitary test
    Trial.Joint(11).sequence          = 'YXZ';
    Euler                             = R2mobileYXZ_array3(Trial.Joint(11).T.full(1:3,1:3,:));
    Trial.Joint(11).Euler.full(1,1,:) = rad2deg(Euler(:,2,:)); % X
    Trial.Joint(11).Euler.full(1,2,:) = rad2deg(Euler(:,1,:)); % Y
    Trial.Joint(11).Euler.full(1,3,:) = rad2deg(Euler(:,3,:)); % Z
